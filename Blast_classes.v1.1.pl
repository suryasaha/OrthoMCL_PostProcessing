...
=head1 NAME
 ...
 orthomcl.Blast_classes.v1.1.pl - prepare GFF files for different hit classes and verify core hits 

=head1 SYNOPSIS

  % orthomcl.Blast_classes.v1.1.pl --xls blast.xls --mxls multi.blast.xls --groups groups.txt --prot file.faa --gff file.gff --title omcl1.run
  
=head1 DESCRIPTION

 Reads in Excel files generated by Blastreport. Did not output files for LAS proteins that 
 had weak hits besides strong hits and the proteins that had multiple strong hits to 1 
 genome. For each LAS protein, it generates GFF+FASTA files for the following categories
...
=head1 VERSION HISTORY
 Version   1.0  INPUT: XLS blast reports, Query <LAS> proteins, Protein GFF, Groups file 
                OUTPUT: GFF+FASTA files for each class
 Version   1.1  INPUT: XLS blast reports, Query <LAS> proteins, Protein GFF, Groups file, nof genomes 
...
my ($xl1,$xl2,$prot,$title,$gff,$orgs);
...
while($rec=<XL2>){
	if($rec=~ /LAS proteins with only weak hits/){last;}
}
for(1..3){$rec=<XL2>;}
while($rec ne "\n"){
	@temp=split("\t",$rec);
...
while($rec=<XL1>){
	if(&get_class($rec,$orgs) eq 'Core'){
		...
		if (!exists $gff{$temp[0]}){print STDERR "$temp[0] core not found\n";} else {print OCGFF $gff{$temp[0]};}
		$ctrs{'Core'}++;
	}
		...
		if (!exists $gff{$temp[0]}){print STDERR "$temp[0] shared not found\n";} else {print OSGFF $gff{$temp[0]};}
		$ctrs{'Shared'}++;
	}
		...
		if (!exists $gff{$temp[0]}){print STDERR "$temp[0] mixed not found\n";} else {print OMGFF $gff{$temp[0]};}
		$ctrs{'Mixed'}++;
	}
}
close(XL1);
