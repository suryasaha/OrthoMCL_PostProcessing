...
=head1 NAME
 ...
 orthomcl.blastreport.v1.2.pl - prepare a Excel report to summarize <LAS> protein hits to labelled <Rhizobiacae> pan-proteome 
...
=head1 DESCRIPTION
 ...
 THE OUTPUT FILES. Also lists the OrthoMCL groups hit by a <LAS> protein.
...
$weakhits,$las_annot_ctr,$las_unannot_ctr,$i,$j,$rec,%ctr,$grp,%groups,%hitgrps,%nohitgrps,$flag,$gen);
...
print OUT "$src protein\t\t";
foreach $i (@orgs){
	print OUT $i,"\t\t";
}
print OUT "\nGI\tDesc\t";
for $i (1..$gen){print OUT "Class\tEval\t";}
print OUT "Groups\n";
print OUT1 "$src protein\t\tHit\nGI\tDesc\tOrg\tClass\tEval\tGroup\n";
...
#		if (@hits>0) {print OUT "\n";}
		...
		@temp= keys %hitgrps;
		foreach $i (@temp){
			delete $nohitgrps{$i}; print OUT $i,"\t";
		}
	}
	else{
		$las_unannot_ctr++;
...
print OUT1 "$src protein\t\tHit\nGI\tDesc\tOrg\tClass\tEval\n";
print OUT1 $onlyweakhits;
print OUT1 "\n\n$src proteins with no hits\n";
print OUT1 "GI\tDesc\n";
print OUT1 $nohits;
print OUT1 "\n\nGroups with no hits from $src proteins\n";
@temp= keys %nohitgrps;
foreach $i (@temp){ print OUT1 "$i\n";}

close(OUT);
close(OUT1);

print STDERR "$las_annot_ctr $src proteins had hits (above and below threshold). $las_unannot_ctr had no hits\n";
