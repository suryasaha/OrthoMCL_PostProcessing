...
=head1 NAME
 ...
 orthomcl.Blast_classes.v1.pl - prepare GFF files for different hit classes and verify core hits 

=head1 SYNOPSIS

  % orthomcl.Blast_classes.v1.pl --xls blast.xls --mxls multi.blast.xls --groups groups.txt --prot file.faa --gff file.gff --title omcl1.run
  
=head1 DESCRIPTION

 Reads in Excel files generated by Blastreport and for each <LAS> protein generates GFF+FASTA 
 files for the following categories
     Core (core and core_paralogous)
     Shared (shared/shared_paralogous)
     Mixed (lineage_specific/_paralogous and missing)
     Weak hits (below blast eval cutoff)
     No hits
 The weakhits set overlaps with other sets right now.
 
=head1 VERSION HISTORY
 Version   1.0  INPUT: XLS blast reports, Query <LAS> proteins, Groups file, Map file 
                OUTPUT: GFF+FASTA files for each class
                    
=head1 NOTE
 NUMBER OF GENOMES IS HARD CODED TO 8 INSIDE GET_CLASS()
...
are mandatory (see below).
   ...
   --gff    <.fna>    Query Refseq GFF file (required)
...
sub get_class{
	...
	#first 2 cols are LAS info, this math does not work since $rec has 19 items, 19th is a empty string??
	...
	if($ctrs{'Core'}==8){return 'Core';}
	elsif($ctrs{'Shared'}==8){return 'Shared';}
	else{return 'Mixed';}
}

my ($xl1,$xl2,$prot,$title,$gff);
...
GetOptions (
	...
	'prot=s' => \$prot,
	'gff=s' => \$gff,
	'title:s' => \$title) or (system('pod2text',$0), exit 1);
...
while($rec=<XL2>){
	if($rec=~ /LAS proteins with weak hits/){last;}
}
for(1..3){$rec=<XL2>;}
while($rec ne "\n"){
	@temp=split("\t",$rec);
...
while($rec=<XL1>){
	if(&get_class($rec) eq 'Core'){
		...
		print OCGFF $gff{$temp[0]};
		$ctrs{'Core'}++;
	}
	elsif(&get_class($rec) eq 'Shared'){
		@temp=split("\t",$rec);
		if(!($faadb->header($temp[0]))){ print STDERR "could not find $temp[0] in $prot\n\n"; exit 1;}
		print OSPROT '>',$faadb->header($temp[0]),"\n"; print OSPROT $faadb->seq($temp[0]),"\n";
		print OSGFF $gff{$temp[0]};
		$ctrs{'Shared'}++;
	}
	elsif(&get_class($rec) eq 'Mixed'){
		@temp=split("\t",$rec);
		if(!($faadb->header($temp[0]))){ print STDERR "could not find $temp[0] in $prot\n\n"; exit 1;}
		print OMPROT '>',$faadb->header($temp[0]),"\n"; print OMPROT $faadb->seq($temp[0]),"\n";
		print OMGFF $gff{$temp[0]};
		$ctrs{'Mixed'}++;
	}
}
close(XL1);
